...
// Package color palette — mirrors the web CalendarView's 12-color set. Events
// are colored by a stable hash of the package name (same algorithm as the web),
// so a given package always maps to the same swatch on both platforms.
const PACKAGE_COLORS: { bg: string; text: string }[] = [
...
/** Identical to the web's getPackageNameHash — stable 32-bit string hash. */
function packageNameHash(name: string): number {
  if (!name) return 0;
  let hash = 0;
...
function packageColor(name: string): { bg: string; text: string } {
