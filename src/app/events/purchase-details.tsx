...
const EventPurchaseDetailsScreen = () => {
  ...
  const balance = Math.max(0, detail.totalAmount - detail.amountPaid);
  ...
  const calendarDraft = buildCalendarEventDraft({
    ...
    } — ${detail.quantity} ticket${detail.quantity > 1 ? "s" : ""}.`,
  });
  ...
  const handleAddToCalendar = async () => {
    ...
    try {
      ...
      if (result.ok) {
        Alert.alert("Added to Calendar", "This ticket was added to your calendar.");
      } else if (result.reason === "permission-denied") {
        Alert.alert(
          "Permission needed",
          "Allow calendar access so this ticket can be saved to your calendar.",
        );
      } else {
        Alert.alert(
          "Couldn't add to calendar",
          result.message ?? "The ticket could not be added to your calendar.",
        );
      }
    } finally {
      ...
      >
        ...
        <SectionCard icon="user" title="Purchase Information">
          ...
          <Text className="text-base font-semibold text-gray-900 dark:text-white">
            {detail.customerName}
            {detail.isGuest ? (
              <Text className="text-xs font-normal text-gray-400"> · Guest</Text>
            ) : null}
          </Text>
          {!!detail.email && (
        ...
        <SectionCard icon="calendar" title="Event Details">
          <InfoRow label="Event" value={detail.eventName} />
          {!!detail.locationName && (
          ...
          {!!detail.purchaseDate && (
            <InfoRow
              ...
              value={formatScheduledDate(detail.purchaseDate)}
            />
          )}
          {!!detail.purchaseTime && (
            <InfoRow
              label="Scheduled Time"
              value={convertTo12Hour(detail.purchaseTime)}
            />
          )}
        </SectionCard>
          ...
          {detail.notes || detail.specialRequests ? (
            <>
              ...
              {!!detail.specialRequests && (
                <View className={detail.notes ? "mt-3" : ""}>
                  <Text className="text-xs text-gray-400 dark:text-gray-500 mb-1">
                    Special Requests
                  </Text>
                  <Text className="text-sm text-gray-700 dark:text-gray-200 leading-5">
