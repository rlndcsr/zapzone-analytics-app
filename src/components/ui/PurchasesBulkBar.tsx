...
export type PurchaseBulkAction =
  | "confirmed"
  | "checked-in"
  | "cancelled"
  | "refunded"
...
type StatusChip = {
  action: Exclude<PurchaseBulkAction, "delete">;
  label: string;
  icon: React.ComponentProps<typeof Feather>["name"];
  tint: string;
};

// The same status options the web bulk bar's "Change Status" select offers.
const STATUS_CHIPS: StatusChip[] = [
  ...
  { action: "refunded", label: "Refund", icon: "rotate-ccw", tint: "#7C3AED" },
];
...
/**
 * Bulk-action toolbar for the Manage Purchases table, mirroring the web admin's
 * bulk bar: a status change (Confirm / Check In / Cancel / Refund) plus Delete,
 * shown only while at least one row is selected. Identical look/behaviour to the
 * BookingsBulkBar — the actions scroll horizontally and `busy` locks the bar.
 */
export function PurchasesBulkBar({
  ...
  count: number;
  busy: PurchaseBulkAction | null;
  onStatus: (status: Exclude<PurchaseBulkAction, "delete">) => void;
  onDelete: () => void;
  onClear: () => void;
}) {
