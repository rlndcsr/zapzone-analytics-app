...
/** Matches the KPI card: icon badge, label, value, subtitle. */
function KpiCardSkeleton({ pulse }: { pulse: SharedValue<number> }) {
  return (
    ...
    >
      ...
      <SkeletonLine pulse={pulse} width="w-10" line="h-8" bar="h-6" className="mt-1" />
      <SkeletonLine pulse={pulse} width="w-16" line="h-4" className="mt-1" />
    </View>
  );
}

/** The KPI summary cards, in a 2-column grid. */
export function FeeSupportKpiSkeleton() {
...
/** Matches FeeSupportCard: name + status pill, amount + meta chips. */
function FeeSupportCardSkeleton({ pulse }: { pulse: SharedValue<number> }) {
  return (
    <View
      className="bg-white dark:bg-neutral-900 rounded-2xl p-4 mb-3 shadow-sm"
      style={CARD_SHADOW}
    >
      {/* name (left) + status pill (right) */}
      ...
      {/* amount + calculation + application chips */}
      <View className="flex-row items-center gap-2 mt-2">
        <SkeletonBlock pulse={pulse} className="w-16 h-6 rounded-lg" />
      ...
      <View className="flex-row items-center justify-between mt-3 pt-3 border-t border-gray-100 dark:border-neutral-800">
        <SkeletonLine pulse={pulse} width="w-28" line="h-4" bar="h-2.5" />
...
/** The fee-support card list placeholder. */
export function FeeSupportListSkeleton() {
  const pulse = usePulse();
  return (
    <View>
      {Array.from({ length: LIST_COUNT }).map((_, i) => (
        <FeeSupportCardSkeleton key={i} pulse={pulse} />
      ))}
    </View>
  );
}
