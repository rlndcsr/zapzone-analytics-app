import React from "react";
import { View } from "react-native";
import type { SharedValue } from "react-native-reanimated";
import { SkeletonBlock, usePulse } from "./SkeletonBlock";

const KPI_COUNT = 4;
const LIST_COUNT = 4;

const CARD_SHADOW = {
  shadowColor: "#000",
  shadowOffset: { width: 0, height: 2 },
  shadowOpacity: 0.05,
  shadowRadius: 8,
  elevation: 2,
} as const;

/** A skeleton bar vertically centered within a text line's height. */
function SkeletonLine({
  pulse,
  width,
  line,
  bar = "h-3",
  className = "",
}: {
  pulse: SharedValue<number>;
  width: string;
  line: string;
  bar?: string;
  className?: string;
}) {
  return (
    <View className={`${line} justify-center ${className}`}>
      <SkeletonBlock pulse={pulse} className={`${width} ${bar}`} />
    </View>
  );
}

/** Matches the KPI card: icon badge, label, value, subtitle. */
function KpiCardSkeleton({ pulse }: { pulse: SharedValue<number> }) {
  return (
    <View
      className="flex-1 bg-white dark:bg-neutral-900 rounded-2xl p-4 m-1.5 shadow-sm"
      style={CARD_SHADOW}
    >
      <SkeletonBlock pulse={pulse} className="w-9 h-9 rounded-xl" />
      <SkeletonLine pulse={pulse} width="w-20" line="h-4" className="mt-3" />
      <SkeletonLine pulse={pulse} width="w-10" line="h-8" bar="h-6" className="mt-1" />
      <SkeletonLine pulse={pulse} width="w-16" line="h-4" className="mt-1" />
    </View>
  );
}

/** The 4 KPI summary cards, in a 2-column grid. */
export function SpecialPricingKpiSkeleton() {
  const pulse = usePulse();
  return (
    <View className="flex-row flex-wrap -mx-1.5">
      {Array.from({ length: KPI_COUNT }).map((_, i) => (
        <View key={i} className="w-1/2">
          <KpiCardSkeleton pulse={pulse} />
        </View>
      ))}
    </View>
  );
}

/** Matches SpecialPricingCard: name + status pill, discount + meta chips. */
function SpecialPricingCardSkeleton({ pulse }: { pulse: SharedValue<number> }) {
  return (
    <View
      className="bg-white dark:bg-neutral-900 rounded-2xl p-4 mb-3 shadow-sm"
      style={CARD_SHADOW}
    >
      {/* name (left) + status pill (right) */}
      <View className="flex-row items-start justify-between mb-2">
        <View className="flex-1 mr-3">
          <SkeletonLine pulse={pulse} width="w-40" line="h-6" bar="h-4" />
          <SkeletonLine pulse={pulse} width="w-28" line="h-4" className="mt-1" />
        </View>
        <SkeletonBlock pulse={pulse} className="w-16 h-6 rounded-full" />
      </View>

      {/* discount + entity chips */}
      <View className="flex-row items-center gap-2 mt-2">
        <SkeletonBlock pulse={pulse} className="w-16 h-6 rounded-lg" />
        <SkeletonBlock pulse={pulse} className="w-20 h-6 rounded-lg" />
      </View>

      {/* meta row + actions */}
      <View className="flex-row items-center justify-between mt-3 pt-3 border-t border-gray-100 dark:border-neutral-800">
        <SkeletonLine pulse={pulse} width="w-24" line="h-4" bar="h-2.5" />
        <View className="flex-row gap-2">
          <SkeletonBlock pulse={pulse} className="w-8 h-8 rounded-lg" />
          <SkeletonBlock pulse={pulse} className="w-8 h-8 rounded-lg" />
        </View>
      </View>
    </View>
  );
}

/** The special-pricing card list placeholder. */
export function SpecialPricingListSkeleton() {
  const pulse = usePulse();
  return (
    <View>
      {Array.from({ length: LIST_COUNT }).map((_, i) => (
        <SpecialPricingCardSkeleton key={i} pulse={pulse} />
      ))}
    </View>
  );
}
