...
describe("isTabRoute", () => {
  ...
  it("does not treat a pushed screen under a tab's name as a tab", () => {
    // This is the case that matters: /calendar has a tab bar beneath it and
    // /bookings/calendar does not, so an overlay anchored to the bottom would
    // float too high on the pushed screen.
    assert.equal(isTabRoute("/bookings/calendar"), false);
    // Profile is pushed over the tabs now that Accounts holds the tab slot.
    assert.equal(isTabRoute("/profile"), false);
